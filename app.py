...
Esta seção demonstra uma possível utilização futura da linguagem InclusaLang.

Os dados apresentados são simulados e possuem finalidade exclusivamente acadêmica.
""")

    recurso = st.selectbox(
        "Escolha um recurso de acessibilidade:",
        [
            "RAMPA",
            "LEITOR_TELA",
            "MATERIAL_ADAPTADO",
    ...
    if st.button("📨 Registrar Solicitação"):
        ...
        st.success(
            f"Solicitação de {recurso} registrada com sucesso!"
        )

    st.subheader("📈 Demandas Simuladas")
    ...
    st.info(
        "Os dados apresentados são simulados para demonstrar uma possível aplicação prática da linguagem InclusaLang."
    )
